...
export function sumPoints(questions: Question[]): number {
    return questions.reduce((acc: number, q: Question) => (acc += q.points), 0);
}
...
export function sameType(questions: Question[]): boolean {
    return [questions.length, 0].includes(
        questions.filter((q: Question) => q.type === "multiple_choice_question")
            .length,
    );
}
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce(
        (arr: Question[], q: Question) =>
            q.id === targetId ?
                (arr = [...arr, q, duplicateQuestion(newId, q)])
            :   [...arr, q],
        [],
    );
}
